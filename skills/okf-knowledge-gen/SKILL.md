...
执行 4 个 Phase：
1. **Discover** — 3 并行 + 1 串行 agent：数据面穷举所有概念（纯事实记录）+ 控制面枚举所有入口并追踪到 IO 边界 + 配置依赖扫描
2. **Plan** — 3 agent（Plan-Data + Plan-Control 并行合成 → Plan-Merge 交叉合并 + Type 分配 + 链接预案 + 文件读取清单）
3. **Generate** — 3 并行 agent 按文件读取负载（轻量/中量/重量）生成 OKF 文档 + 交叉链接
4. **Verify** — 断链检测、一致性校验、实现断言复验、index 生成

**预计耗时**：20 万行仓库约 25-35 分钟，50 万行级仓库约 40-55 分钟。
...
4. 手动审查标注为"推断"的断言
5. 数据面/控制面分离 + Plan-Merge 交叉验证会自动暴露盲区和冲突——查看 `cross_check_results` 了解哪些概念可能被遗漏
...
- 50 万行级单仓库：当前管线已支持（数据面穷举 + 按负载分组生成）
- 50 万行以上：考虑拆分为多个独立 Bundle（按模块/领域拆分）
